...
sub get_cached_file_VCF {
    ...
    if ($self->cache()->exists($key) && !$self->forbid_cache()) {
        ...
        print STDERR "--> [start] get info from protocols...\n";
        foreach (@$protocol_ids) {
            my $protocol = CXGN::Genotype::Protocol->new({
                bcs_schema => $self->bcs_schema,
                nd_protocol_id => $_,
        ...
        print STDERR "    [finish] ...get info from protocols\n";
        ...
        print STDERR "--> [start] init_genotype_iterator()...\n";
        $self->init_genotype_iterator();
        print STDERR "    [finish] ...init_genotype_iterator()\n";

        #VCF should be sorted by chromosome and position
        print STDERR "--> [start] sort markers...\n";
        no warnings 'uninitialized';
        @all_marker_objects = sort { $a->{chrom} cmp $b->{chrom} || $a->{pos} <=> $b->{pos} || $a->{name} cmp $b->{name} } @all_marker_objects;
        @all_marker_objects = $self->_check_filtered_markers(\@all_marker_objects);
        print STDERR "    [finish] ...sort markers\n";

        my $counter = 0;
        my $usingGT;
        print STDERR "--> [start] build vcf file...\n";
        while (my $geno = $self->get_next_genotype_info) {
            ...
            if ($counter == 0) {
                ...
                print STDERR "--> HEADER LINES:\n";
                print STDERR Dumper $genotype_string;
            ...
            print STDERR "--> DATA STRING FOR GENOTYPE: $genotype_id\n";
            print STDERR Dumper $genotype_data_string;

            write_file($tempfile, {append => 1}, $genotype_string);
            $counter++;
        }
        print STDERR "    [finish] ...build vcf file\n";

        print STDERR "--> [start] transpose vfc file...\n";
        my $transpose_tempfile = $tempfile . "_transpose";
        my $cmd = CXGN::Tools::Run->new({
            backend => $backend_config,
            submit_host => $cluster_host_config,
        ...
        print STDERR "    [finish] ...transpose vcf file\n";

        #Get synonyms of the accessions
        print STDERR "--> [start] lookup sample synonyms...\n";
        ...
        print STDERR "    [finish] ...lookup sample synonyms\n";

        print STDERR "--> [start] write final file...\n";
        ...
        close $out_copy;
        $file_handle = $self->cache()->handle($key);
        print STDERR "--> [finish] ...write final file\n";
    }

    print STDERR "    [finish] GET CACHED VCF FILE\n";
    return $file_handle;
}
