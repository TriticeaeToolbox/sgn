...
=head1 SYNOPSIS
    ...
    load_phenotypes_spreadsheet_csv.pl -H [dbhost] -D [dbname] -U [dbuser] -P [dbpass] -N trial_name -i [infile] -a [archive path on server] -d [datalevel] -u [username] -t [timestamps included] -o [overwrite previous values]
...
perl bin/load_phenotypes_spreadsheet_csv.pl -D cass -H localhost -U postgres -P postgres -N trial_name -u nmorales -i ~/Downloads/combined_counts.csv -a /export/prod/archive/ -d plants
...
our ($opt_H, $opt_D, $opt_U, $opt_P, $opt_N, $opt_i, $opt_d, $opt_u, $opt_t, $opt_o);

getopts('H:D:U:P:N:i:a:d:u:t:o:');
...
my $chado_schema = Bio::Chado::Schema->connect(
...
my $validate_type = "phenotype spreadsheet simple";
...
my $basename = basename($upload);

my $uploader = CXGN::UploadFile->new({
   tempfile => $filename_phenotype,
...
print STDERR "$upload $subdirectory $basename $sp_person_id\n";
my $archived_filename_with_path = $uploader->archive();
print STDERR "$archived_filename_with_path\n";
...
my $validate_file = $parser->validate($validate_type, $archived_filename_with_path, $timestamp_included, $data_level, $chado_schema);
if (!$validate_file) {
...
my $parsed_file = $parser->parse($validate_type, $archived_filename_with_path, $timestamp_included, $data_level, $chado_schema);
if (!$parsed_file) {
...
my $temp_file_nd_experiment_id = '$archive_path/copy.txt';
