...
#opendir(my $dir, "/home/production/public/upload_pheno_fix") or die "Cannot open directory $!";
while (readdir $dir) {
    if (/([^\.]+)\.([a-z]+)/) {
        $base = $1;
        $ext = $2;
	    ...
	    #$cmd = 'perl t3-create-trials.pl -H breedbase_db -D cxgn_triticum -p ' . $dbpass . ' -N ' . $base . ' -U cbirkett -i /home/production/public/upload_pheno_fix/';
