...
if($form->data('submit') && ($form->data('submit') eq 'Search') || ($form->data('random') && $form->data('random') eq 'yes')) {
  ...
  if (@protos = $form->data_multiple('protos')) {
    if ($protos[0] ne "Any") {
      $query = "select nd_protocol_id from nd_protocol where name = ?";
      my $search_results = $dbh->prepare("$query");
      $search_results->execute($protos[0]);
      if (my @row_ary = $search_results->fetchrow_array()) {
        $protocol = $row_ary[0];
        print STDERR "protocol_id = $protocol\n";
      } else {
        print STDERR "Error: bad protocol\n";
        return;
      }
    }
  }
